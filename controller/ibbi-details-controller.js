...
IBBI.search = async (req, res) => {
  try {
    ...
    const response = await axios.post(`${Karza.API_BASE_URL}/v3/ibbi-details`, requestBody, {
      headers: {
    ...
    await saveLog(USER_ID, "IBBI", responseTime, response.data.statusCode === 101 ? "success" : "failed", requestBody);
